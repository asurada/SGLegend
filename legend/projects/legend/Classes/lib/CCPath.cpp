...
float CCPath::getStroke()
{
    return _stroke;
}

void CCPath::setStroke(float stroke)
{
...
CCPath::CCPath(const char *filePath, float stroke, int pointLimit)
: _texture(NULL)
{
    setColor(ccWHITE);
    setOpacity(255);
...
CCPath* CCPath::create(const char *filePath, float stroke, int pointLimit)
{
    CCPath* pRet = new CCPath(filePath, stroke, pointLimit);
    pRet->autorelease();
    return pRet;
}

void CCPath::populateVertices()
{
    // head
    int i = 0;
    _vertices[0] = _path[i];
...
void CCPath::push(const CCPoint &point)
{
...
void CCPath::popLastOne(float delta)
{
    pop(1);
...
float CCPath::getDrainInterval()
{
    return _drainInterval;
}

void CCPath::setDrainInterval(float drainInterval)
{
    _drainInterval = drainInterval;
    if (drainInterval > 0) {
        schedule(schedule_selector(CCPath::popLastOne), drainInterval);
    } else {
        unschedule(schedule_selector(CCPath::popLastOne));
    }
}

void CCPath::autoCleanup()
{
    _autoCleanup = true;
    if (_drainInterval <= 0) {
